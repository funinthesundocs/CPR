-- Deploy compute_verdict function as tracked migration
-- Aggregates votes for a case and upserts into verdict_results
-- Excludes flagged votes from all calculations
...
DECLARE
    ...
    v_result JSONB;
BEGIN
    -- Count total votes (excluding flagged)
    SELECT COUNT(*)
    INTO v_total_votes
    FROM votes
    WHERE case_id = p_case_id AND flagged = false;
    ...
    -- Calculate average guilt score (excluding flagged)
    ...
    -- Count nominal damages approvals (excluding flagged)
    SELECT
    ...
    -- Sum punitive damages (excluding flagged)
    SELECT COALESCE(SUM(punitive_amount), 0)
    INTO v_total_punitive
    FROM votes
    WHERE case_id = p_case_id AND punitive_amount IS NOT NULL AND flagged = false;
