...
| i18n key mismatches invisible at build time | A missing translation key returns the raw key string at runtime with zero build warnings — grep the translation file for every t() call after adding new keys to a component | Seed | 2026-02-20 | 1 |
...
| Query DB before parsing field content | Before writing logic that parses a text field (location, address, tags) for structured values, query actual DB rows to see what the field really contains — fields named like structured data often hold free-text prose at runtime | Seed | 2026-03-03 | 0 |
